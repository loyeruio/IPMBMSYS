  ...
  <div class="app-container">
      ...
      <el-col :span="19" :xs="24" class="content-col">
        <div class="content-container" style="height: calc(100vh - 120px); overflow-y: auto; overflow-x: hidden;">
          <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
            ...
            <el-form-item label="会议负责人" prop="organizerId">
              <el-input
            ...
            <el-form-item label="举办部门" prop="branchId">
              <el-input
          ...
          <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
              <el-button
                ...
                :disabled="multiple"
                @click="handleDelete"
                v-hasPermi="['common:common:remove']"
              >删除</el-button>
            </el-col>
            <el-col :span="1.5">
              <el-button
                ...
                size="mini"
                @click="handleExport"
                v-hasPermi="['common:common:export']"
              >导出</el-button>
            </el-col>
            <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
          </el-row>
          <el-table v-loading="loading" :data="commonList" @selection-change="handleSelectionChange" :expand-row-keys="expandedRows" row-key="id">
            <el-table-column type="expand">
              <template slot-scope="props">
                <ExpandCard
                  :row-data="props.row"
                  participants-title="与会人员"
                  edit-button-text="管理人员"
                  :show-batch-button="true"
                  batch-button-text="批量考勤"
                  attachments-title="会议附件"
                  :attachments="getAttachmentList(props.row.attachments)"
                  attendance-title="参会情况"
                  :show-attendance-button="true"
                ...
                >
                  <template #participants="{ data }">
                    <ParticipantsTable
                      ...
                      :participants="meetingParticipants[data.id] || []"
                      :user-list="userList"
                      :editable="false"
                      :attendance-status-options="dict.type.attendance_status"
                      @attendance-change="handleBatchAttendanceChange"
                      @signin-change="handleBatchSignInChange"
            ...
            <el-table-column label="会议类型" align="center" prop="meetingType" width="140">
              <template slot-scope="scope">
                <dict-tag :options="dict.type.meeting_type" :value="scope.row.meetingType"/>
              </template>
            </el-table-column>
            <el-table-column label="会议主题" align="center" prop="title" :show-overflow-tooltip="true"/>
            <el-table-column label="举办部门" align="center" prop="branchId" width="120">
            ...
            <el-table-column label="会议内容" align="center" prop="description" :show-overflow-tooltip="true">
              <template slot-scope="{ row }">
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="举办部门" prop="branchId">
          <treeselect v-model="form.branchId" :options="deptOptions" :show-count="true" placeholder="请选择举办部门" />
        </el-form-item>
...
<script>
import { listCommon, getCommon, delCommon, addCommon, updateCommon } from "@/api/meeting/meeting";
...
// 引入会议参与人员API
import { listMeetingparticipant, getMeetingparticipant, addMeetingparticipant, updateMeetingparticipant, delMeetingparticipant } from "@/api/participant/meetingparticipant";
  ...
  data() {
    return {
      ...
      rules: {
        ...
        branchId: [
          { required: true, message: "举办部门不能为空", trigger: "blur" }
        ],
        startTime: [
  ...
  methods: {
    ...
    async getList() {
      ...
      try {
        ...
        this.fillDeptInfo(this.commonList);
        
        // 加载每个会议的参与人员数据
        await this.loadParticipantsForMeetings(this.commonList);
        console.log('meeting_type:', this.dict.type.meeting_type)
        console.log('meetings_status:', this.dict.type.meetings_status)
        this.loading = false;
      } catch (error) {
    ...
    async loadParticipantsForMeetings(meetingList) {
      try {
        for (const meeting of meetingList) {
          const participants = await this.getParticipantsForMeeting(meeting.id);
          this.$set(this.meetingParticipants, meeting.id, participants);
        }
      } catch (error) {
    ...
    /** 获取单个会议的参与人员数据 */
    async getParticipantsForMeeting(meetingId) {
      try {
        const response = await listMeetingparticipant({ meetingId: meetingId });
        return response.rows || [];
      } catch (error) {
        console.error(`获取会议${meetingId}的参与人员失败:`, error);
        return [];
      }
    },
    // 在methods中添加
    ...
    resetQuery() {
      ...
      this.queryParams.branchId = null; // 重置部门筛选
      this.handleQuery();
    },
    // 多选框选中数据
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('common/common/export', {
        ...this.queryParams
    ...
    getDeptTree() {
      deptTreeSelect().then(response => {
        this.deptOptions = response.data;
        // console.log(response.data); // 可以保留或移除
      });
    },
    /** 处理负责人选择 */
    ...
    handleSendNotification(row) {
      // 获取当前会议的参与人员
      this.getParticipantsForMeeting(row.id).then(participants => {
    ...
    async submitParticipants() {
      ...
      const meetingId = this.participantsForm.id;
        
      try {
        this.$modal.loading("正在处理参与人员数据...");
        
        // 获取当前参与人员
        const currentParticipants = await this.getParticipantsForMeeting(meetingId);
        ...
        if (toDelete.length > 0) {
          ...
          if (deleteIds.length > 0) {
            await delMeetingparticipant(deleteIds.join(','));
          }
        }
        ...
        if (toAdd.length > 0) {
          ...
          if (addData.length > 0) {
            await Promise.all(addData.map(item => addMeetingparticipant(item)));
          }
        }
        ...
        if (updateData.length > 0) {
          await Promise.all(updateData.map(item => updateMeetingparticipant(item)));
        }
    ...
    getAttachmentList(attachments) {
      ...
      try {
        ...
        // 处理出勤记录
    handleEditAttendance(row) {
      this.handleEditParticipants(row);
    },
    ...
    updateRowAttachments(id, attachments) {
      ...
      updateCommon(updateData).then(response => {
    ...
    async handleBatchAttendance(row) {
      ...
      try {
        const response = await listMeetingparticipant({ meetingId: row.id });
        // 处理新的数据结构
        this.currentParticipants = response.rows || [];
        this.batchAttendanceDialogVisible = true;
      } catch (error) {
        console.error('获取参与人员数据失败:', error);
        this.$modal.msgError("获取参与人员数据失败");
      }
    },
    ...
    applyBatchAttendance() {
      ...
      this.$message.success("批量设置出勤状态成功，请点击保存按钮提交");
    },
    ...
    applyBatchSignIn() {
      ...
      this.$message.success("一键签到成功，请点击保存按钮提交");
    },
    
    // 处理批量考勤变化
    handleBatchAttendanceChange(data) {
      // 更新当前参与人员数据
      this.currentParticipants = data;
    },
    
    // 处理批量签到状态变化
    handleBatchSignInChange(data) {
      // 更新当前参与人员数据
      this.currentParticipants = data;
    },
    ...
    async submitBatchAttendance() {
      ...
      try {
        ...
        const updateData = this.currentParticipants
          ...
          .map(participant => ({
            id: participant.id,
            meetingId: this.currentEditingId,
        ...
        console.log('批量更新数据:', updateData); // 添加调试日志
        
        // 使用Promise.all并发请求，提高效率
        if (updateData.length > 0) {
          const results = await Promise.all(updateData.map(item => updateMeetingparticipant(item)));
        ...
        await this.getList();
        // 重新获取当前会议的参与人员数据
        this.currentParticipants = await this.getParticipantsForMeeting(this.currentEditingId);
      } catch (error) {
        this.$modal.closeLoading();
        this.$modal.msgError("批量考勤记录失败: " + (error.message || error));
    ...
    submitAttendance() {
      ...
      const participants = this.$refs.attendanceTable.getParticipantsData();
      const updateData = {
    ...
    async handleEditParticipants(row) {
      this.currentEditingId = row.id;
      this.currentParticipants = await this.getParticipantsForMeeting(row.id);
    ...
    async updateParticipant(participant) {
      try {
        ...
        // 更新参与人员信息
        await updateMeetingparticipant(participant);
    ...
    // 确认参与人员修改
    async handleParticipantsConfirm(participantsData) {
      try {
        this.$modal.loading("正在处理参与人员数据...");
        
        // 获取当前参与人员
        const currentParticipants = await this.getParticipantsForMeeting(this.currentEditingId);
        const currentUserIds = currentParticipants.map(p => p.userId);
        
        // 找出需要删除的参与人员
        const toDelete = currentParticipants.filter(p => !participantsData.some(newP => newP.userId === p.userId));
        
        // 找出需要添加的参与人员
        const toAdd = participantsData.filter(p => !currentUserIds.includes(p.userId));
        
        // 找出需要更新的参与人员
        const toUpdate = participantsData.filter(p => {
          return currentUserIds.includes(p.userId) && p.id;
        });
        ...
        if (toDelete.length > 0) {
          const deleteIds = toDelete.map(p => p.id).filter(id => id);
          if (deleteIds.length > 0) {
            await Promise.all(deleteIds.map(id => delMeetingparticipant(id)));
          }
        }
        
        // 批量添加新的参与人员
        if (toAdd.length > 0) {
          const addData = toAdd.map(p => ({
            meetingId: this.currentEditingId,
            userId: p.userId,
            userName: p.userName,
            deptName: p.deptName,
            signInStatus: p.signInStatus || '0',
            attendanceStatus: p.attendanceStatus || '0',
            remark: p.remark || ''
          }));
        ...
        if (toUpdate.length > 0) {
          await Promise.all(toUpdate.map(item => updateMeetingparticipant(item)));
        }
        ...
        // 刷新参与人员数据
        const updatedParticipants = await this.getParticipantsForMeeting(this.currentEditingId);
        this.$set(this.meetingParticipants, this.currentEditingId, updatedParticipants);
