  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="carouselList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="轮播图类型" align="center" prop="type">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.system_carousel_type" :value="scope.row.type"/>
        </template>
      </el-table-column>
      <el-table-column label="排序" align="center" prop="sort" />
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_carousel_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column label="播放开始时间" align="center" prop="startTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="播放结束时间" align="center" prop="endTime" width="180">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="轮播图类型" prop="type">
          <el-select v-model="form.type" placeholder="请选择轮播图类型">
            <el-option
              v-for="dict in dict.type.system_carousel_type"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
        ...
        <el-form-item label="状态" prop="status">
          <el-radio-group v-model="form.status">
            <el-radio
              v-for="dict in dict.type.sys_carousel_status"
              :key="dict.value"
              :label="dict.value"
            >{{dict.label}}</el-radio>
          </el-radio-group>
...
export default {
  name: "Carousel",
  dicts: ['sys_carousel_status', 'system_carousel_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
