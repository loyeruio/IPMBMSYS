  ...
  <view class="participants-container">
    ...
    <view class="header-actions">
      <view class="class-info">
        <text class="class-title">{{ classDetail.title }}</text>
        <text class="participant-count">参课人员 ({{ participants.length }})</text>
      </view>
    ...
    <view v-if="participants.length === 0" class="empty-state">
      ...
      <button class="add-first-btn" @click="showAddParticipant">添加第一个参课人员</button>
    </view>
    ...
    <uni-popup ref="participantPopup" type="bottom">
      <view class="popup-content">
        <view class="popup-header">
          <text class="popup-title">{{ isEdit ? '编辑参课人员' : '添加参课人员' }}</text>
        </view>
...
import { 
  listClassparticipant, 
  addClassparticipant, 
  updateClassparticipant, 
  delClassparticipant 
} from '@/api/participant/classparticipant'
  ...
  methods: {
    ...
    async deleteParticipant(participant, index) {
      try {
        await uni.showModal({
          title: '确认删除',
          content: `确定要删除参课人员"${participant.userName}"吗？`,
          confirmText: '删除',
          confirmColor: '#f56c6c'
        })
    ...
    async saveParticipant() {
      ...
      try {
        if (this.isEdit) {
          ...
          const response = await addClassparticipant(participantData)
