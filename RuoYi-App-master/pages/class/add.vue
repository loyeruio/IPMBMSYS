  ...
  <view class="add-container">
    ...
    <view class="form-container">
      <view class="form-header">
        <text class="form-title">创建党课信息</text>
      </view>
...
<script>
import { addClass } from '@/api/class/class'
    ...
    async submitForm() {
      ...
      try {
        ...
        const submitData = {
          ...
          branchId: parseInt(this.formData.branchId),
          duration: this.formData.duration ? parseInt(this.formData.duration) : null,
          status: '0'
        }
        
        await addClass(submitData)
        this.$modal.msgSuccess('党课创建成功')
        setTimeout(() => {
          this.goBack()
        }, 1500)
      } catch (error) {
        console.error('创建党课失败:', error)
